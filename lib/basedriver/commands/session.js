...
commands.createSession = async function createSession (jsonwpDesiredCapabilities, jsonwpRequiredCaps, w3cCapabilities) {
  ...
  if (w3cCapabilities) {
    ...
    if (jsonwpDesiredCapabilities) {
      log.debug(`W3C capabilities ${_.truncate(JSON.stringify(w3cCapabilities))} and MJSONWP desired capabilities ${_.truncate(JSON.stringify(jsonwpDesiredCapabilities))} were provided`);
    }
    ...
    if (jsonwpDesiredCapabilities && !_.isPlainObject(w3cCapabilities)) {
      ...
      log.info('Changing the W3C capabilties...');
